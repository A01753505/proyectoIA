# FUNCIONES PARA LA LIMPIEZA DE DATOS Y CREACIÓN DE DATAFRAMES
...
# Separar datos numéricos y categóricos en dos DataFrames
...
# Tratamiento de datos CATEGÓRICOS con KNN
def cat_KNN_cleaning(df_cat, col):
    # Imputación predictiva o basada en KNN
    df_cat[col] = df_cat[col].fillna(False)

    return df_cat

# Tratamiento de datos NUMÉRICOS con KNN
...
# Realiza una codificación dependiendo de la columna del DataFrame
...
# Combina los DataFrames de datos numéricos y categóricos en uno solo
...
# Leer datos
# df = pd.read_csv("Modelo/train.csv")            # Para el DataFrame de entrenamiento
df = pd.read_csv("Modelo/test.csv")               # Para el DataFrame de pruebas

# X = df.drop(columns = ["Name", "Transported"])  # Para el DataFrame de entrenamiento
X = df.drop(columns = ["Name"])                   # Para el DataFrame de pruebas
...
# df_new.to_csv('DF.csv', index=False)            # Para el DataFrame de entrenamiento
df_new.to_csv('DF_test.csv', index=False)       # Para el DataFrame de pruebas